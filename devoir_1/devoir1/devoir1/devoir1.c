...
int main() {
	...
	assert(nb_jours_mois(2, 2024) == 29);
...
while (1) {																									//Structure de controle while, pour interrompre le programe avec des instructions break.  			
	...
	nbr_annee = valider_entier("\nAnnee (entre 1900 et 2024), 0 pour annuler: ", 1900, 2024);				 
	...
	else nbr_mois = valider_entier("\nMois (entre 1 et 12), 0 pour annuler: ", 1, 12);						// Tout autres années entre 1900 et 2023.			
	if (nbr_mois == 0) break;
	
	//Section saisie du jour.
	joursMax = nb_jours_mois(nbr_mois, nbr_annee);															//Obtient jours totals contenus dans le mois selon l'année.
	char message[50];																						//Concatenation du message pour y ajouter la variable des jours valides.			
	...
	printf(" %d.", nbr_annee);
	break;																									//Ceci est la fin du programme.
}
#endif
